...
interface FoodDao {
    ...
    @Query("SELECT * FROM food_entries WHERE date >= :startOfDay AND date < :endOfDay AND isWater = 1 ORDER BY date DESC")
    fun getWaterListForDate(startOfDay: Long, endOfDay: Long): Flow<List<FoodEntry>>

    @Query("SELECT * FROM food_entries")
    suspend fun getAllFoodList(): List<FoodEntry>

    @Query("SELECT * FROM meal_presets")
    suspend fun getAllPresetsList(): List<MealPreset>

    @Query("SELECT * FROM product_metadata")
    suspend fun getAllProductsList(): List<ProductMetadata>
